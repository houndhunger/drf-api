"""
Views for managing followers in the API, including following and unfollowing
users.
"""
...
class NotificationUpdate(generics.UpdateAPIView):
    ...
    def perform_update(self, serializer):
        # Get the notification from the URL parameter
        notification = self.get_object()
        # Ensure the notification belongs to the logged-in user
        if notification.owner != self.request.user:
            raise NotFound(
                "Notification not found or does not belong "
    ...
    def get_object(self):
        """
        Override to ensure we fetch the correct notification based "
        "on URL parameter.
        """
    ...
    def post(self, request, notification_id, *args, **kwargs):
        ...
        try:
            ...
            notification = Notification.objects.get(
                id=notification_id, owner=request.user
                )
...
class NotificationDeleteAllReadView(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        """
        notifications = Notification.objects.filter(
            owner=request.user, is_read=True
            )
        if notifications.exists():
            notifications.delete()  # Delete read notifications
            return Response(
                {"detail": "All read notifications deleted."},
                status=status.HTTP_204_NO_CONTENT
                )
        return Response(
            {"detail": "No read notifications found."},
            status=status.HTTP_404_NOT_FOUND
            )
